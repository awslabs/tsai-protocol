...
Beyond the API, a Trust Authority publishes four documents at well-known locations: its signing keys at `/.well-known/jwt-vc-issuer`, a type-metadata document per `vct`, an agent-and-operator status list, and a signed operational report at `/.well-known/tsai-ta-status`. A first deployment issues identity and reputation signals; compliance and assurance follow as an authority builds those evaluations. There are no tiers.
