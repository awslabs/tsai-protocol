...
TSAI does not define how platforms use trust signals to make access decisions (platform policy), how Trust Authorities evaluate agents (TA methodology), or how agents implement their functionality (agent implementation).
...
- MUST comply with Section 5 (Security and Privacy)
- MAY choose which Trust Authorities to trust (platform policy)
- MAY choose how to interpret trust signals (platform policy)
...
**Platform:** Service or system that agents access. Verifies agent credentials to make access decisions.
...
**Challenge-Response:** Protocol where platform sends a random nonce and agent signs it to prove freshness and prevent replay attacks.
