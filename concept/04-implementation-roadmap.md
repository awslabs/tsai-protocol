...
Stage 1 adds production-grade security. It adds the compliance and assurance signals, the verification-strength policy of ADR 018 (a Service-Provider nonce challenge and a tighter freshness window where the risk warrants), request binding for state-changing actions (ADR 014), the schema that requires the identity floor and Type Metadata that controls selective disclosure (ADR 015), and the agent or operator block via the Token Status List, and it lays the ground for a multi-TA ecosystem.
